...
export default function MarketplaceSectionClient({ items, section, title, description }: Props) {
    ...
    <div className="min-h-screen bg-forge-navy">
      <section className="relative overflow-hidden border-b border-forge-border">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-16 md:py-24">
          <div className="mx-auto max-w-3xl text-center">
            <h1 className="text-4xl font-extrabold tracking-tight text-text-primary md:text-5xl">{title}</h1>
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => (
            <Link key={item.slug} href={`/agentmarketplace/${section}/${item.slug}`} className="group flex flex-col rounded-2xl border border-forge-border bg-forge-card p-6 transition-all hover:border-data-cyan hover:shadow-lg hover:shadow-data-cyan/10">
              {item.image && (
              ...
              <h2 className="mt-2 text-xl font-bold text-text-primary transition-colors group-hover:text-data-cyan">{item.title}</h2>
