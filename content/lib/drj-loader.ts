...
const CONTENT_DIR = path.join(process.cwd(), "content", "drj");
...
function loadPost(slug: string): DrJPost | undefined {
...
export function getAllDrJPosts(): DrJPost[] {
  ...
  return files
    ...
    .filter((p): p is DrJPost => p !== undefined)
    .sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime();
    });
}

export function getDrJPostBySlug(slug: string): DrJPost | undefined {
  return loadPost(slug);
}

export function getAllDrJCategories(): string[] {
