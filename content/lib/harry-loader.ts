...
const CONTENT_DIR = path.join(process.cwd(), "content", "harrys-desk");
...
function loadPost(slug: string): HarryPost | undefined {
...
export function getAllHarryPosts(): HarryPost[] {
  ...
  return files
    ...
    .filter((p): p is HarryPost => p !== undefined)
    .sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime();
    });
}

export function getHarryPostBySlug(slug: string): HarryPost | undefined {
  return loadPost(slug);
}

export function getAllHarryCategories(): string[] {
