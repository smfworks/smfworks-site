...
const CONTENT_DIR = path.join(process.cwd(), "content", "the-ledger");
...
function loadPost(slug: string): LedgerPost | undefined {
...
export function getAllLedgerPosts(): LedgerPost[] {
  ...
  return files
    ...
    .filter((p): p is LedgerPost => p !== undefined)
    .sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime();
    });
}

export function getLedgerPostBySlug(slug: string): LedgerPost | undefined {
  return loadPost(slug);
}

export function getAllLedgerCategories(): string[] {
