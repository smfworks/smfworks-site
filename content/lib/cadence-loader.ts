...
const CONTENT_DIR = path.join(process.cwd(), "content", "the-cadence");
...
function loadPost(slug: string): CadencePost | undefined {
...
export function getAllCadencePosts(): CadencePost[] {
  ...
  return files
    ...
    .filter((p): p is CadencePost => p !== undefined)
    .sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime();
    });
}

export function getCadencePostBySlug(slug: string): CadencePost | undefined {
  return loadPost(slug);
}

export function getAllCadenceCategories(): string[] {
